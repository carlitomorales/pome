<?php include('header.php'); ?>
             ...
             <div class="row-fluid">
            	<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2>List Kegiatan Efisiensi Energi Pada Peralatan Pengguna Energi Utama</h2>
						<div class="box-icon">
                      ...
                      <table class="table table-striped table-bordered bootstrap-datatable" id="datatable1">
                          <thead>
                              <tr>
                                  ...
                                  <th colspan="2" width="20%" style="text-align:center">Penghematan yang Diperoleh</th>
                                  <th rowspan="2" style="text-align:center">Emisi (tCO<sub>2</sub>)</th>
                                  <th rowspan="2"  style="text-align:center">Keterangan</th>
                                  <th rowspan="2" width="10%" style="text-align:center">Actions</th>
                              </tr>
                              <tr>
            ...
            <div class="row-fluid">
            	<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2>Input Kegiatan Efisiensi Energi Pada Peralatan Pengguna Energi Utama</h2>
						<div class="box-icon">
                        	<h2>Tahun <?php echo date("Y")-1;?></h2>
						</div>
					</div>
					<div class="box-content">
                    	...
                    	<form id="frmAdd" class="form-horizontal" action="" method="post">
							<div class="control-group">
                                <label class="control-label" for="id_peralatan">Peralatan Pengguna Energi Utama</label>
                                <div class="controls">
                                      <select name="id_peralatan" id="id_peralatan">
                                          <option selected value="">-- Pilih --</option>
                                            <?php
                                                    foreach($data_peralatan as $row){
														if ($row->JenisPeralatan== "Lain-Lain")
														{ echo '<option value="'.$row->id_peralatan_pemanfaat_energi.'">'.$row->jenis_peralatan_lain." ( ".$row->JenisPeralatan." ) ( ".$row->JenisEnergi." - ".$row->detail_jenis_energi." )"."</option>"; }
														else
                           ...
                           <div class="control-group">
                            <label class="control-label" for="realisasi_gjoule"> Penghematan yang Diperoleh *)</label>
                            <div class="controls">
                           ...
                           <div class="control-group">
                            <label class="control-label" for="realisasi_emisi"> Emisi</label>
                            <div class="controls">
                              <div class="input-append">
             ...
             <div class="row-fluid">	
                <div class="box span12">
					<div class="box-header well" data-original-title>
						<h2>List Kegiatan Efisiensi Lain</h2>
						<div class="box-icon">
                        	<h2>Tahun <?php echo date("Y")-1;?></h2>
						</div>
					</div>
            ...
            <div class="row-fluid">
            	<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2>Input Kegiatan Efisiensi Lain</h2>
						<div class="box-icon">
                        	<h2>Tahun <?php echo date("Y")-1;?></h2>
						</div>
					</div>
             ...
             <div class="row-fluid">
                <div class="box span12">
                	<div class="box-content">
						...
						<span class="btn btn-primary" onClick="document.location='<?php echo base_url();?>rencanakonservasinew'" style="float:right">Next</span>
                    </div>
                </div>
             </div>
<?php include('footer.php'); ?>
	...
	$(document).ready(function(){
		...
		oTable = $("#datatable1").dataTable({
			...
			"bProcessing": true,
			"bServerSide": true,
			"sAjaxSource": "<?php echo base_url();?>kegiatankonservasinew/get_list",
			"sServerMethod" : "POST",
			...
			"aoColumnDefs":[
				...
				
				{
					...
					"fnRender": function(obj){
						...
						{return '<div align="right">'+obj.aData[14]+'&nbsp;('+obj.aData[9]+') </br> ('+obj.aData[10]+' - '+obj.aData[11]+')</div>';}
						else
						{return '<div align="right">'+obj.aData[9]+' </br> ('+obj.aData[10]+' - '+obj.aData[11]+')</div>';}
		...
		$(".btnedit").live('click', function(){
			var id = $(this).attr('id');
			var url = "<?php echo base_url();?>kegiatankonservasinew/getDetail/"+id; // the script where you handle the form input.
			$.post(url, '', function(data){
				...
				if(rowdata){
					...
					var urlFE = "<?php echo base_url();?>kegiatankonservasinew/getFaktorEmisi/"+rowdata.id_peralatan_pemanfaat_energi;
		...
		$("#addrow").click(function(){
			...
			if(!$.isNumeric($("#realisasi_gjoule").val()))
			{
				$("#divAlert").removeClass();
				$("#divAlert").addClass('alert alert-error');
				$("#divAlert").html("Realisasi (Gjoule) harus numerik.");
				$("#realisasi_gjoule").focus();
				return false;					
			}
			...
			var url = "<?php echo base_url();?>kegiatankonservasinew/save"; // the script where you handle the form input.
		...
		$('#id_peralatan').live('change', function(){
			...
			var url = "<?php echo base_url();?>kegiatankonservasinew/getFaktorEmisi/"+id;
			var faktor_emisi;
			var jenis_peralatan;
			var jenis_energi;
		...
		$(".btndelete").live('click', function(){
			...
			if(conf){
				var url = "<?php echo base_url();?>kegiatankonservasinew/hapus/"+id; // the script where you handle the form input.
				$.post(url, '', function(data){
				   oTable.fnDraw();
				    $("#id_kegiatan_konservasi").val('');
				    $("#kegiatan").val('');
		...
		oTable1 = $("#datatable2").dataTable({
			...
			"bProcessing": true,
			"bServerSide": true,
			"sAjaxSource": "<?php echo base_url();?>kegiatankonservasinew/get_list_lain",
			"sServerMethod" : "POST",
			"sDom": "<'row-fluid'<'span6'l><'span6'f>r>t<'row-fluid'<'span12'i><'span12 center'p>>",
			"sPaginationType": "bootstrap",
			"oLanguage": {
		...
		$(".btneditLain").live('click', function(){
			...
			$("#divAlert1").html('');
			var id = $(this).attr('id');
			var url = "<?php echo base_url();?>kegiatankonservasinew/getDetailLain/"+id; // the script where you handle the form input.
			$.post(url, '', function(data){
				objResp = $.parseJSON(data);
				rowdata = objResp[0];
				if(rowdata){
					$("#id_kegiatan_konservasi_lain").val(rowdata.id_kegiatan_konservasi_lain);
		...
		$("#addrow1").click(function(){
			...
			var url = "<?php echo base_url();?>kegiatankonservasinew/saveLain"; // the script where you handle the form input.
		...
		$(".btndeleteLain").live('click', function(){
			...
			$("#divAlert1").html('');
			id = $(this).attr('id');
			conf1 = confirm("Hapus data kegiatan efisiensi lain ini?");
			if(conf1){
				var url = "<?php echo base_url();?>kegiatankonservasinew/hapusLain/"+id; // the script where you handle the form input.
				$.post(url, '', function(data){
				   oTable1.fnDraw();
				   $("#id_kegiatan_konservasi_lain").val('');
				   $("#id_kegiatan_lain").val('');
