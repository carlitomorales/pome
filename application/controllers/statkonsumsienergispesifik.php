...
class statkonsumsienergispesifik extends CI_Controller {
	...
	{
		$this->load->view('statkonsumsienergispesifik', $this->data);
	}
	...
	public function getlinedata(){
		...
		foreach($arrtahun as $rtahun){
						$sql.="SELECT ".$rtahun." AS tahun, Z.* from (select max(rpt.kons) as 'Tidak Efisien', AVG(rpt.kons) as 'Rata-Rata', min(rpt.kons) as 'Paling Efisien' from ( select cast(IFNULL((IFNULL((select sum(if((`tahun` = $rtahun && (id_perusahaan=p.IdPerusahaan)),`gjoule`,0)) from t_pemakaian_energi),0)/IFNULL((select sum(if((`tahun` = $rtahun && (id_perusahaan=p.IdPerusahaan)),`jumlah`,0)) from t_jenis_produksi),0)),0) as decimal(8,2)) as kons,p.IdPerusahaan as id_perusahaan from tm_perusahaan as p where p.IdSubsektor=$sub_sektor  ) as rpt where rpt.kons<>0 ) as Z";
			$sql .= ' UNION ';
		} 
		}
		...
		if($tipe_perusahaan == 2){
			foreach($arrtahun as $rtahun){
			$sql.="SELECT ".$rtahun." AS tahun,Z.* from (select max(rpt.kons) as 'Tidak Efisien', AVG(rpt.kons) as 'Rata-Rata', min(rpt.kons) as 'Paling Efisien' from (
select cast(IFNULL((IFNULL((select sum(if((`tahun` = $rtahun && (id_perusahaan=p.IdPerusahaan)),`gjoule`,0)) from t_pemakaian_energi),0)/IFNULL((select sum(if((`tahun` = $rtahun && (id_perusahaan=p.IdPerusahaan)),`luas_bangunan`,0)) from t_luas_bangunan),0)),0) as decimal(8,2)) as kons,p.IdPerusahaan as id_perusahaan from tm_perusahaan as p
where p.IdSubsektor=$sub_sektor ) as rpt where rpt.kons<>0 ) as Z";
$sql .= ' UNION ';
