...
class Chartperalatanpemanfaat extends CI_Controller {
	...
	{
		$this->load->view('chartperalatanpemanfaat', $this->data);
	}
	...
	public function getpiedata(){
		...
		if($sub_sektor==0)
		{
			$this->db->select("a.id_jenis_peralatan, b.JenisPeralatan AS jenisperalatan, SUM(a.gjoule) AS persen ", false)
		...
		foreach($result as $row){
			...
			if($sub_sektor==0)
			{
				$this->db->select("b.JenisEnergi AS jenisperalatan, SUM(a.gjoule) AS persen ", false)
				->from("t_peralatan_pemanfaat_energi AS a")
				->join("tm_jenisenergi AS b", "a.id_jenis_energi=b.IdJenisEnergi")			
				->join("tm_perusahaan AS tmp", "a.id_perusahaan=tmp.IdPerusahaan")
				->join("(SELECT IdPerusahaan FROM t_statussubmit WHERE tahun=".$tahun." AND status=2) AS sts", "tmp.IdPerusahaan=sts.IdPerusahaan")
				->group_by("b.JenisEnergi")
				->where("a.tahun",$tahun)
				->where("tmp.TypePerusahaan",$tipe_perusahaan)
