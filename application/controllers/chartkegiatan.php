...
class chartkegiatan extends CI_Controller {
	...
	{
		$this->load->view('chartkegiatan', $this->data);
	}
	...
	public function getpiedata(){
		...
		if($sub_sektor==0)
		{
			$this->db->select("a.id_jenis_peralatan, b.JenisPeralatan AS jenisperalatan, count(*) AS persen ", false)
			->from("t_peralatan_pemanfaat_energi AS a")
			->join("t_kegiatan_konservasi_new AS t", "a.id_peralatan_pemanfaat_energi=t.id_peralatan_pemanfaat_energi")	
			->join("tm_jenisperalatan AS b", "a.id_jenis_peralatan=b.IdJenisPeralatan")			
			...
			->group_by("a.id_jenis_peralatan, b.JenisPeralatan")
			->where("a.tahun",$tahun)
		...
		foreach($result as $row){
			...
			if($sub_sektor==0)
			{
				$this->db->select("t.jenis_kegiatan AS jenisperalatan, count(*) AS persen ", false)
				->from("t_peralatan_pemanfaat_energi AS a")
				->join("t_kegiatan_konservasi_new AS t", "a.id_peralatan_pemanfaat_energi=t.id_peralatan_pemanfaat_energi")	
				->join("tm_jenisenergi AS b", "a.id_jenis_energi=b.IdJenisEnergi")			
				->join("tm_perusahaan AS tmp", "a.id_perusahaan=tmp.IdPerusahaan")
				->join("(SELECT IdPerusahaan FROM t_statussubmit WHERE tahun=".$tahun." AND status=2) AS sts", "tmp.IdPerusahaan=sts.IdPerusahaan")
				->group_by("t.jenis_kegiatan")
				->where("a.tahun",$tahun)
				->where("tmp.TypePerusahaan",$tipe_perusahaan)
				->where("a.id_jenis_peralatan",$id_jenis_peralatan);
			}
			else
			{
				$this->db->select("t.jenis_kegiatan AS jenisperalatan, count(*) AS persen ", false)
