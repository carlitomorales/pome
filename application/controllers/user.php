...
//			$this->load->view('user_detailperusahaan', $this->data);
	...
	public function add_save()
	{
		...
		$emailesdm = $this->config->item('email_pic_esdm');
	...
	public function add_kirim_pesan()
	{
		...
		if ($this->session->userdata('privilege_id')=='2' ){
			if ($this->input->post('template')=='template1'){
				...
				//$emailesdm = 'subdit_dka@yahoo.co.id';
				...
				$headers = "From: ".$emailesdm."\r\n";
				$headers .= "Reply-To: ".$emailesdm."\r\n";
				$headers .= "CC: ".$emailesdm."\r\n";
                ...
                $url_hit=  $this->config->item('api_email');
                $urlx = $url_hit."from=".$fromx."&to=".$tox."&subject=".$subjectx."&nourut=".$no_urutx."&tanggal=".$tanggalx."&namap=".$nama_perusahaanx."&alamatp=".$alamat_perusahaanx."&username=".$usernamex."&emailesdm=".$email_esdmx."&pesan=".$pesanx;
...
//		$urlx = "http://dev.creativenesia.com/cikini/sendmail?from=".$fromx."&to=".$tox."&subject=".$subjectx."&message=".$messagex;
		...
		$fromx1 = urlencode($emailesdm);
		$tox1 = urlencode($to1);
		$subjectx1 = urlencode($subject1);
		$messagex1 = urlencode($message1);


        $urlx1 = $url_hit."from=".$fromx1."&to=".$tox1."&subject=".$subjectx1."&message=".$messagex1;
