...
class Reportrencanakegiatankonservasienergiadmin extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('reportrencanakegiatankonservasienergiadmin', $this->data);
	}
	...
	public function get_list()
	{
		...
		if ($nama_perusahaan != "")
		{
			if ($sub_sektor == 0)
			{
				$this->datatables->select("No,NamaPerusahaan, Peralatan ,Kegiatan, DetailKegiatan, TargetGjoule, TargetPersen, TargetEmisi", false)
				->from("(select @s:=@s+1 No,tmp.NamaPerusahaan AS NamaPerusahaan, t.JenisPeralatan as Peralatan ,t.jenis_kegiatan as Kegiatan, t.detail_kegiatan as DetailKegiatan, SUM(t.target_gjoule) AS TargetGjoule, SUM(t.target_persen) AS TargetPersen, SUM(target_emisi) AS TargetEmisi FROM (t_rencana_konservasi_new AS t) JOIN tm_perusahaan AS tmp ON t.id_perusahaan=tmp.IdPerusahaan JOIN t_statussubmit AS tsb on tmp.idperusahaan=tsb.idperusahaan, (SELECT @s:= 0) AS s  WHERE tsb.status=2 and t.tahun = $tahun AND tmp.TypePerusahaan= '$tipe_perusahaan' AND tmp.NamaPerusahaan like '%".$nama_perusahaan."%' group by tmp.NamaPerusahaan, t.tahun, t.jenis_kegiatan, t.JenisPeralatan ) as TblReport");
			}
			else {
				$this->datatables->select("No,NamaPerusahaan, Peralatan ,Kegiatan, DetailKegiatan, TargetGjoule, TargetPersen, TargetEmisi", false)
	...
	public function exportexcel()
	{
		...
		if ($nama_perusahaan != "")
		{
			if ($sub_sektor == 0)
			{
				$this->db->select("No,NamaPerusahaan, Peralatan ,Kegiatan, DetailKegiatan, TargetGjoule, TargetPersen, TargetEmisi", false)
				->from("(select @s:=@s+1 No,tmp.NamaPerusahaan AS NamaPerusahaan, t.JenisPeralatan as Peralatan ,t.jenis_kegiatan as Kegiatan, t.detail_kegiatan as DetailKegiatan, SUM(t.target_gjoule) AS TargetGjoule, SUM(t.target_persen) AS TargetPersen, SUM(target_emisi) AS TargetEmisi FROM (t_rencana_konservasi_new AS t) JOIN tm_perusahaan AS tmp ON t.id_perusahaan=tmp.IdPerusahaan JOIN t_statussubmit AS tsb on tmp.idperusahaan=tsb.idperusahaan, (SELECT @s:= 0) AS s  WHERE tsb.status=2 and t.tahun = $tahun AND tmp.TypePerusahaan= '$tipe_perusahaan' AND tmp.NamaPerusahaan like '%".$nama_perusahaan."%' group by tmp.NamaPerusahaan, t.tahun, t.jenis_kegiatan, t.JenisPeralatan ) as TblReport");
			}
			else {
				$this->db->select("No,NamaPerusahaan, Peralatan ,Kegiatan, DetailKegiatan, TargetGjoule, TargetPersen, TargetEmisi", false)
		...
		$this->excel->to_excel($query, 'report_rencana_konservasi_energi_'.date("YmdHis")); 
	}
	...
	public function getpiedata(){
		...
		->from("t_rencana_konservasi_new AS t")
	...
	public function getlinedata(){
		...
		foreach($tahun as $rtahun){
			...
			foreach($jenis_kegiatan as $rjeniskegiatan){
				$sql .= "(SELECT IFNULL(SUM(target_gjoule),0) FROM t_rencana_konservasi_new AS t INNER JOIN tm_perusahaan AS p ON t.id_perusahaan=p.IdPerusahaan WHERE t.tahun=".$rtahun['tahun']." AND t.kegiatan=".$rjeniskegiatan['id_kegiatan']." AND p.IdSubsektor=".$sub_sektor.") AS '".$rjeniskegiatan['jenis_kegiatan']."',";
			}
			$sql = substr($sql, 0, -1);
			$sql .= ' UNION ';
		}
