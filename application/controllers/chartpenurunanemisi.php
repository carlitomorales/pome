...
class Chartpenurunanemisi extends CI_Controller {
	...
	{
		$this->load->view('chartpenurunanemisi', $this->data);
	}
	...
	public function getpiedata(){
		...
		foreach($arrsubsektor as $idsubsektor){
			//echo $idsubsektor."<br/>";
			$sql .= " SELECT (select NamaSubsektor from tm_subsektor where IdMSubsektor=".$idsubsektor.") as NamaSubsektor, (SELECT SUM(realisasi_emisi) FROM t_kegiatan_konservasi_new WHERE id_perusahaan IN (SELECT IdPerusahaan FROM tm_perusahaan WHERE idSubsektor=".$idsubsektor.") AND tahun=".$tahun." AND id_perusahaan IN (SELECT IdPerusahaan FROM t_statussubmit WHERE tahun=".$tahun." AND `status`=2)) AS penghematan UNION";
		}
		$sql = substr($sql,0, strlen($sql)-5);
		
		/*
		$this->db->select("a.NamaSubsektor as NamaSubsektor,
(SELECT SUM(realisasi_emisi) FROM t_kegiatan_konservasi_new WHERE id_perusahaan IN (SELECT IdPerusahaan FROM tm_perusahaan WHERE idSubsektor=a.IdMSubsektor) AND tahun=".$tahun." AND id_perusahaan IN (SELECT IdPerusahaan FROM t_statussubmit WHERE tahun=".$tahun." AND `status`=2)) AS penghematan ", false)
