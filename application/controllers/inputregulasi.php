...
class Inputregulasi extends CI_Controller {
	...
	{
		//Dropdown master 
		$this->load->view('inputregulasi', $this->data);
	}
			...
			if($_FILES['thumbnail']['tmp_name'] != ''){
				...
				else
				{
					...
					if($id_regulasi == ''){
						...
						$id_regulasi = $this->regulasi_model->insert($data);
					}else{						
						$data = array(
							'nama_regulasi' => $nama_regulasi,
							//'publish_date' => $publish_date,
						...
						$this->regulasi_model->edit($id_regulasi,$data);
					}
					...
					if($this->db->trans_status() === FALSE)
					{
						...
						$status = "error";
						$msg = "Informasi lain gagal disimpan, silakan coba lagi.";
					}
					else
					{
						$status = "success";
						$msg = "Informasi lain berhasil disimpan.";
					}
				}
	...
	function getDetail($id){
		$rowdata = $this->regulasi_model->getById($id);
	...
	function hapus($id){
		...
		$msg = '';
		$this->db->trans_start();
		$this->regulasi_model->del($id);
		$this->db->trans_complete();
		if($this->db->trans_status() === FALSE)
		{
			$status = "error";
			$msg = "Informasi lain gagal dihapus, silahkan coba lagi.";
		}else{
			$status = "success";
			$msg = "Informasi lain berhasil dihapus.";
		}
		echo json_encode(array('status' => $status, 'msg' => $msg));
	}
}
