...
class Auditenergi extends CI_Controller {
	...
	function save(){	
		...
		if(empty($_POST['ruang_lingkup']) && $_POST['status_audit'] == '1'){
			$status = 'error';
			$msg = 'Silakan isi ruang lingkup audit energi.';
		}
	...
	function get_list_rekomendasi(){
		$id_perusahaan = $this->session->userdata('id_perusahaan');
		$tahun = date("Y")-1;
		$this->datatables->select('c.JenisPeralatan as jenis_peralatan, a.id_peralatan_pemanfaat_energi, rekomendasi, potensi_penghematan, jenis_rekomendasi, rencana_implementasi, id_rekomendasi_audit',false)
			->where('a.id_perusahaan = '.$id_perusahaan)
			->where('a.tahun = '.$tahun)
