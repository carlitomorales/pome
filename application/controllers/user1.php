...
//			$this->load->view('user_detailperusahaan', $this->data);
	...
	{
        $status_0 = $this->config->item('status_0');
        $status_1 = $this->config->item('status_1');
        $status_2 = $this->config->item('status_2');
        $status_3 = $this->config->item('status_3');
        var_dump('$status_0');
		$this->load->helper('user_helper');
		$this->datatables->select("real_name, b.tahun,(CASE b.status WHEN 0 THEN '".$status_0."' WHEN 1 THEN '".$status_1."' WHEN 2 THEN '".$status_2."' WHEN 3 THEN '".$status_3."' END) AS submit,a.idperusahaan");
