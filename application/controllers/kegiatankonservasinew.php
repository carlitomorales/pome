...
class Kegiatankonservasinew extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('kegiatankonservasinew_model');
	...
	public function index()
	{
		...
		$this->data['data_kegiatan'] = $this->kegiatankonservasinew_model->get_fordropdowndetail();
		$this->data['data_kegiatan_lain'] = $this->kegiatankonservasinew_model->get_fordropdowndetaillain();
		$this->load->view('inputkegiatankonservasinew', $this->data);
	}
	...
	function save(){
		...
		if($id_kegiatan_konservasi != ''){
			...
			$this->kegiatankonservasinew_model->edit($id_kegiatan_konservasi,$data);
		}else{
			$data = array(
				'id_peralatan_pemanfaat_energi' => $id_peralatan,
				'JenisPeralatan' => $jenis_peralatan,
			...
			$newid = $this->kegiatankonservasinew_model->insert($data);
		}		
	...
	function saveLain(){
		...
		if($id_kegiatan_konservasi_lain != ''){
			$data = array(
				'id_kegiatan_konservasi_lain' => $id_kegiatan_konservasi_lain,
				'kegiatan' => $kegiatan,
				'detail_kegiatan' => $detail_kegiatan,
				'jenis_kegiatan' => $jenis_kegiatan,
				'update_by' => $this->session->userdata('username'),
			...
			$this->kegiatankonservasinew_model->editLain($id_kegiatan_konservasi_lain,$data);
		}else{
			$data = array(
				'kegiatan' => $kegiatan,
				'detail_kegiatan' => $detail_kegiatan,
			...
			$newid = $this->kegiatankonservasinew_model->insertLain($data);
		}		
		$this->db->trans_complete();
		if($this->db->trans_status() === FALSE){
			$status = "error";
			$msg = "Terjadi kesalahan saat menyimpan data kegiatan efesiensi energi lain, silakan coba lagi.";
		}else{
			$this->stepsubmit();
			$status = "success";
			$msg = "Penyimpanan data kegiatan efesiensi energi lain berhasil.";
		}
		echo json_encode(array("status"=>$status,"msg"=>$msg));
	}
	...
	function hapus($id){
		$this->db->trans_start();
		$this->kegiatankonservasinew_model->del($id);
		$this->db->trans_complete();
	}
	function hapusLain($id){
		$this->db->trans_start();
		$this->kegiatankonservasinew_model->delLain($id);
		$this->db->trans_complete();
	}
	function get_list(){
		$id_perusahaan = $this->session->userdata('id_perusahaan');
	...
	function getDetail($id){
		$rowdata = $this->kegiatankonservasinew_model->getById($id);
		echo json_encode($rowdata);
	}
	function getDetailLain($id){
		$rowdata = $this->kegiatankonservasinew_model->getByIdLain($id);
		echo json_encode($rowdata);
	}
	function getFaktorEmisi($id){
		$idp = $this->session->userdata('id_perusahaan');
		$rowdata = $this->kegiatankonservasinew_model->getKonservasiByIdPeralatanPemanfaat($id,$idp);
		echo json_encode($rowdata);
	}
}
